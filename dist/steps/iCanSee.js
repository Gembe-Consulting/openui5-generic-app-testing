/*! openui5-generic-app-testing 2018-12-02 */
...
(function() {
    ...
    module.exports = {
        docs: {
            ...
            examples: [ "I can see btnNext", "I can see btnNext with text 'next' in Detail view", "I can see lblTitle in Overview view", "I can see txtInput with value '1234ABCD' in Main view", "I can see txtInput with value containing '123' in Main view", "I can see txtInput with value containing '123'", "I can see boolSwitch with state is 'true'", "I can see boolSwitch with state is 'false' in Sub view", "I can see the 21st sap.m.Button control deeply nested inside grpButtons with text 'Apples' in Main view", "I can see the sap.m.Button control deeply nested inside grpButtons" ]
        },
        icon: "show",
        regexp: new RegExp([ "^I can see\\s+", "(", "(the( first| last| \\d+?st| \\d+?rd| \\d+?th| \\d+?nd)?( ([a-zA-Z]+[.])+[a-zA-Z]+)? control\\s(deeply|directly) nested inside ([a-zA-Z0-9]+))|", "([a-zA-Z0-9]+)", ")", "(\\swith\\s([^\\s]+)\\s(being\\s|containing\\s|starting\\swith\\s|ending\\swith\\s)?'(.*?)')?", "(\\s+in\\s+(.+?)\\sview)?\\s*$" ].join("")),
